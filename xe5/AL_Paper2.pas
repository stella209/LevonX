...
Uses
    ...
    Extctrls, Messages, VCL.Dialogs, Forms, NewGeom, DGrafik, B_Spline, Szoveg,
    Szamok, StObjects, Clipper;

Type

  TInOutRec = record           // Metszési pont rekord
...
//                        10,FGraphTitle)
  ...
  begin
    ...
    begin
      ...
      begin
           DrawBmp.Canvas.Pen.Color := clBlack;
           if FC.GetPointRec(I).Selected then
           begin
              DrawBmp.Canvas.Brush.Color := clBlue;
              DrawBmp.Canvas.Rectangle(PA^[I].x-fPointWhdth,PA^[I].y-fPointWhdth,
                PA^[I].x+fPointWhdth,PA^[I].y+fPointWhdth);
           end
           else
           begin
              DrawBmp.Canvas.Brush.Color:=clLime;
      ...
      if FC.OutBase then
           DrawBmp.Canvas.Brush.Color := clLime;
      DrawBmp.Canvas.Ellipse(PA^[0].x-fPointWhdth-2,PA^[0].y-fPointWhdth-2,
                PA^[0].x+fPointWhdth+2,PA^[0].y+fPointWhdth+2);
  ...
  DrawSelectedFrame;
  If CursorCross then DrawMouseCross(oldMovePt,pmNotXor);
  painting := False;
  if Assigned(fChangeWindow) then
     fChangeWindow(Self,fOrigo, fZoom, MousePos)
end;
except
  Exit;
end;
...
begin
  ...
  if (ssLeft in Shift) then begin
     ...
     1: begin
          ...
          FCurve.ClearPoints;
          if (ssCtrl in Shift) then // négyzetté deformálás
          begin
     ...
     if pFazis>-1 then begin
        ...
        if XYCompulsion and (pFazis>0) then      // XY kényszer
     ...
     begin
       ...
       if (ActionMode = amNone) and (CurveMatch or CPMatch or CurveIn) then begin
             FCurve := FCurveList.Items[CPCurve];
  ...
  if (ssLeft in Shift) then begin
     ...
     amNone           :
       begin
          ...
          if (Shift = [ssCtrl,ssLeft]) or (Shift = [ssShift,ssLeft]) then
          begin
             if SelectedFrame.Visible then
             begin
                case  SelectedFrame.ActualNode of
                -1,9: SelectedFrame.Move(XToW(x)-XToW(oldMovePt.x),YToW(Height-oldMovePt.y)-YToW(Height-y));
                0..3: SelectedFrame.SetNode(SelectedFrame.ActualNode,SToW(MovePt));
  ...
  if (DrawMode<>dmNone) and (pFazis>0) {and (not Zooming)} then begin
     ...
     if MaxPointsCount>=FCurve.FPoints.Count then begin
        ...
        begin
          ...
          if FDefiniedLength then
          begin
               if XYCompulsion then      // XY kényszer
               begin
                    if (pFazis>0) then begin
                       if pp.Y=yy then
                          if pp.X>xx then
...
//  MouseInOut:=0;
  ...
  if fCoordHint then begin
     ...
     THintLabel.Top  := Y+24;
     THintLabel.Caption := Format('%6.1f : %6.1f',[xx,yy]);
     THintLabel.Update;
  end;
  ...
  If (CurveMatch or CPMatch or CurveIn) and (Shift = []) then begin
     ...
     If CPMatch then
        Hintstr := ' ['+IntToStr(CPCurve)+'] '+fCurve.Name+'  ( '+IntToStr(CPIndex)+' / '+IntToStr(FCurve.Count)+' ) ';
...
begin
 ...
 if Paning then Screen.Cursor := crDefault;
  ...
  if FDrawmode<>dmNone then SignedNotCutting;
...
var i,n: integer;
    ...
    procedure InitSelArray;
    var ii,jj: integer;
    begin
       jj := 0;
       SetLength(selArray,GetSelectedCount);
       for ii:=0 to Pred(FCurveList.Count) do begin
         if Curves[ii].Selected and (Curves[ii].shape=dmPolyLine) then
         begin
           selArray[jj] := ii;
           Inc(jj);
         end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          p1 := Curves[selArray[ii]].Points[Cuv.Count-1];
...
// Az 1 vagy 0 pontot tartalmazó alakzatok törlése
...
begin
  ...
  PontsuritesAll(2);
  InitParentObjects;
    // Eltoljuk a child objektumokat és felfûzzük, majd visszatoljuk
    SelectChildObjects;
    MoveSelectedCurves(-2000,0);
      Strip;
    MoveSelectedCurves(2000,0);
    While Strip>0 do;
  DeleteInvisibleCurves;
...
begin
   For i:=0 to Pred(FCurvelist.Count) do begin
       if Curves[i].Count>3 then begin
          ...
          Vektorisation(0.1,Curves[i].Contour);
       end;
   end;
//   Repaint;
end;
...
Try
  CR := ContourRadius;
  ContourRadius := ContourRadius-0.5;
  SetAllContour;
  for i := 0 to Pred(FCurvelist.Count) do
  begin
