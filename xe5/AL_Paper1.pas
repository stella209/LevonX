...
Uses
    ...
    Extctrls, Messages, Dialogs, NewGeom, Forms,
    DGrafik, B_Spline, Szoveg, Szamok, StObjects, Clipper;
...
Type
  ...
  private
    ...
    procedure SetAutoUndo(const Value: boolean);
  ...
  public
    ...
    function SaveGraphToFile(var FileName: string): Boolean;
  ...
  published
    ...
    property AutoUndo : boolean read fAutoUndo write SetAutoUndo;
...
//                        10,FGraphTitle)
  ...
  begin
    case KK of
    0: FCurve:=FCurveList.Items[H];
    1: begin
         Cuv:=FCurveList.Items[H];
         FCurve := Cuv.Contour;
       end;
    end;
    ...
    if FCurve.Visible and (FCurve.FPoints.Count > 0)
       then
    begin
      DrawBmp.Canvas.Pen.Style := psSolid;
      DrawBmp.Canvas.Pen.Width:=1;
...
function TALSablon.IsNode(p: TPoint2d; Radius: double; var idx: integer): boolean;
var i: integer;
    d: double;
    re: TRect2d;
begin
...
function TAlSablon.SaveGraphToFile(var FileName: string): Boolean;
var
  FileStream: TFileStream;
...
function TALSablon.OutLineObject(AIndex: Integer; delta: real): TCurve;
begin
...
begin
  ...
  For i:=0 to Pred(FCurveList.Count) do begin
      ...
      Selected := BaseCurve;
      szaz := Trunc(100*(i/FCurveList.Count));
      if Assigned(FPlan) then FPlan(Self,0,szaz);
...
*)
//============================================================================
procedure TALSablon.ElkerulesAB(Var eCurve: TCurve);
...
(*
   ...
   For i:=0 to Pred(FCurvelist.Count) do begin
       Cuv:=FCurveList.Items[i];
//       FCurve.SetContour(ContourRadius);
...
procedure TALSablon.SetAutoUndo(const Value: boolean);
begin
  fAutoUndo := Value;
end;
...
//           if TempCurve.Contour=nil then TempCurve.VisibleContour := True;
           TempCurve := GetClipperContour(BaseCurve,ContourRadius);
//           TempCurve := ObjectContour(BaseCurve,ContourRadius);
    ...
    begin
         ...
         For ii:=0 to Pred(FCurveList.Count) do begin
             ...
             if fc.Shape=dmPolygon then begin
                ...
                R := Rect2d(R.X1-delta,R.y1-delta,R.x2+delta,R.y2+delta);
                if IsSzakaszNegyszogMetszes(A,B,R) then begin
                   Contour := GetClipperContour(fc,ContourRadius);
                   if Contour.Count=0 then begin
//                      fc.SetContour(ContourRadius);
                      Vektorisation(0.1,Contour);
                   end;
                   if (Contour.IsCutLine(A,B,dd)) then begin
                      mpArr[pCount].idx := ii;
                      mpArr[pCount].d   := dd;
                      Inc(pCount);
                   end;
...
//                If Cuv.Contour = nil then begin
                   // Kontúrképzés az átmetszett poligon körül
//                   TempCurve := Cuv.Contour;
                   TempCurve := GetClipperContour(Cuv,ContourRadius);
                   Vektorisation(0.1,TempCurve);
                   ContourOptimalizalas(BaseCurve);
//                end else
//                   TempCurve := Cuv.Contour;
