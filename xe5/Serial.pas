...
function COMOpenEx(PortName: string): Boolean;
...
function COMGetInDb(fh : Thandle): Integer;
...
begin
  ...
  if fh=INVALID_HANDLE_VALUE then begin
     pName:=UpperCase('\\.\'+CommPort);
     fh:= CreateFile(PChar(pName),
...
begin
  Result:= False;
  pName:=UpperCase('\\.\'+PortName);
  hCom:= CreateFile(PChar(pName),
                    GENERIC_READ or GENERIC_WRITE,
                    0,
                    nil,
                    OPEN_EXISTING,
...
begin
  ...
  begin
    ClearCommError(fh, dwErrorCode, @stat);
    Result:= stat.cbInQue;
  end;
end;
