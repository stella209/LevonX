...
type
  ...
  TFastDIB = class
    ...
    procedure SafeSetPixel(y,x:Integer;c:TFColor);
    procedure SafeSetPixelB(y,x:Integer;c:Byte);
    procedure SetPixelBxy(x,y:Integer;c:Byte);
    ...
    procedure SetTextColorB(Color:byte);
    ...
    procedure LineBresenham(x0,y0,x1,y1 : integer; Color : TFColor);
    procedure LineBresenhamB(x0,y0,x1,y1 : integer; Color : byte);
    function  DrawEllipse(CX, CY, XRadius, YRadius : integer; Color : tfColor; Fill : boolean = false) : integer;
    function  DrawEllipseB(CX, CY, XRadius, YRadius : integer; Color : byte) : integer;
    procedure DrawFrame(X1,Y1,X2,Y2 : integer; Color : TFColor);
    procedure DrawFrameB(X1,Y1,X2,Y2 : integer; Color : byte);
    procedure DrawArc(CX, CY, XRadius, YRadius, X1,Y1, X2,Y2 : integer; Color : tfColor);
    procedure FloodFill(XP,YP : integer; Old, New : tfColor);

    procedure LineBresenhamMirrorY(x0,y0,x1,y1 : integer; Color : TFColor);
    procedure LineBresenhamBMirrorY(x0,y0,x1,y1 : integer; Color : byte);
    function  DrawEllipseMirrorY(CX, CY, XRadius, YRadius : integer; Color : tfColor; Fill : boolean = false) : integer;
    function  DrawEllipseBMirrorY(CX, CY, XRadius, YRadius : integer; Color : byte) : integer;
    procedure DrawFrameMirrorY(X1,Y1,X2,Y2 : integer; Color : TFColor);
    procedure DrawFrameBMirrorY(X1,Y1,X2,Y2 : integer; Color : byte);
    procedure DrawArcMirrorY(CX, CY, XRadius, YRadius, X1,Y1, X2,Y2 : integer; Color : tfColor);
    procedure FloodFillMirrorY(XP,YP : integer; Old, New : tfColor);
    ...
    procedure SetColor(n,cr,cg,cb:Integer);
    procedure LoadFromResource(hInst:HINST; Name : string);
  end;
...
procedure TFastDIB.SetPixelBxy(x,y:Integer;c:Byte);
begin SetPixelB(Height-1-y,x,c);
...
begin
  ...
  if fBpp = 8 then begin
     for I := 0 to 255 do SetColor(I,I,I,I);
     UpdateColors;
  end;
end;
...
procedure TFastDIB.SetTextColorB(Color:byte);
begin
   Windows.SetTextColor(hDC,Colors^[Color].b shl 16 + Colors^[Color].g shl 8 + Colors^[Color].r);
end;
...
procedure TFastDIB.SetColor(n,cr,cg,cb:Integer);
begin
   Colors^[n].r := cr;
   Colors^[n].g := cg;
   Colors^[n].b := cb;
...
procedure TFastDIB.SafeSetPixelB(y,x:Integer;c:Byte);
begin
   if (x>=0) and (x<BWidth) and (y>=0) and (y<AbsHeight) then SetPixelB(y,x,c);
end;

procedure TFastDIB.SafeSetPixel(y,x:Integer;c:TFColor);
begin
   if (x>=0) and (x<Width) and (y>=0) and (y<AbsHeight) then SetPixel(y,x,c);
end;

procedure TFastDIB.LineBresenham(x0,y0,x1,y1 : integer; Color : TFColor);
var dx,dy,stepx,stepy : integer;
    fraction          : integer;
begin
   dy := y1 - y0;
   dx := x1 - x0;
   ...
   dy := dy shl 1;
   dx := dx shl 1;
   SafeSetPixel(y0,x0,Color);

   if (dx > dy) then begin
//      fraction := dy - (dx shr 1);              // same as 2*dy - dx
      fraction := 2*dy - dx;              // same as 2*dy - dx
...
//      fraction := dx - (dy shr 1);
      fraction := 2*dx - dy;              // same as 2*dy - dx
...
procedure TFastDIB.LineBresenhamB(x0,y0,x1,y1 : integer; Color : byte);
var dx,dy,stepx,stepy : integer;
    fraction          : integer;
begin
   dy := y1 - y0;
...
function TFastDIB.DrawEllipseB(CX, CY, XRadius, YRadius : integer; Color : byte) : integer;
var X, Y ,DotCount         : integer;
...
begin
   SafeSetPixelB(CY+Y, CX+X, color); {point in quadrant 1}
   SafeSetPixelB(CY+Y, CX-X, color); {point in quadrant 2}
   SafeSetPixelB(CY-Y, CX-X, color); {point in quadrant 3}
   SafeSetPixelB(CY-Y, CX+X, color); {point in quadrant 4}
end;

begin
   Result := 0;
   DotCount := 0;
   if XRadius = 0 then begin
      LineBresenhamB(CX,CY-YRadius,CX,CY+YRadius,color);
      exit;
   end else if YRadius = 0 then begin
      LineBresenhamB(CX-XRadius,CY,CX+XRadius,CY,color);
      exit;
   end;
   ...
   while (StoppingX <= StoppingY) do begin {2nd set of points}
         Plot4EllipsePoints(X,Y);
         inc(X);
         inc(StoppingX, TwoBSquare);
         inc(EllipseError, XChange);
         inc(XChange,TwoBSquare);
         if ((2*EllipseError + YChange) > 0) then begin
            dec(Y);
...
function TFastDIB.DrawEllipse(CX, CY, XRadius, YRadius : integer; Color : tfColor; Fill : boolean = false) : integer;
var X, Y ,DotCount         : integer;
    XChange, YChange       : integer;
    EllipseError           : integer;
    TwoASquare, TwoBSquare : integer;
...
begin
   if Fill then begin
      LineBresenham(CX-X, CY-Y, CX+X, CY-Y, color);
      LineBresenham(CX-X, CY+Y, CX+X, CY+Y, color);
   end else begin
      SafeSetPixel(CY+Y, CX+X, color); {point in quadrant 1}
      SafeSetPixel(CY+Y, CX-X, color); {point in quadrant 2}
      SafeSetPixel(CY-Y, CX-X, color); {point in quadrant 3}
      SafeSetPixel(CY-Y, CX+X, color); {point in quadrant 4}
   end;
end;
...
begin
   ...
   DotCount := 0;
   if XRadius = 0 then begin
      LineBresenham(CX,CY-YRadius,CX,CY+YRadius,color);
      exit;
   end else if YRadius = 0 then begin
      LineBresenham(CX-XRadius,CY,CX+XRadius,CY,color);
      exit;
   end;
...
procedure TFastDib.DrawArc(CX, CY, XRadius, YRadius, X1,Y1, X2,Y2 : integer; Color : tfColor);
var X, Y                   : integer;
    XChange, YChange       : integer;
    EllipseError           : integer;
    TwoASquare, TwoBSquare : integer;
...
procedure Plot(Y,X : integer);
begin
   if (X >= X1) and (X <= X2) and (Y >= Y1) and (Y <= Y2) then
      SafeSetPixel(Y, X, color);
end;

procedure Plot4EllipsePoints(X,Y : integer);
begin
   Plot(CY+Y, CX+X); {point in quadrant 1}
   Plot(CY+Y, CX-X); {point in quadrant 2}
   Plot(CY-Y, CX-X); {point in quadrant 3}
   Plot(CY-Y, CX+X); {point in quadrant 4}
end;
...
procedure TFastDib.FloodFill(XP,YP : integer; Old, New : tfColor);
const MAX_STACK = 1024;
var X1,Y1 : integer;
    L,R   : array [0..MAX_STACK] of TPoint;
    LS,RS : integer;
    LF,RF : boolean;

function Compare(C1, C2 : tfColor) : boolean;
begin
  Result := (C1.r = C2.r) and (C1.g = C2.g) and (C1.b = C2.b);
end;    

begin
   if (XP < 0) or (XP >= Width) then Exit;
   if (YP < 0) or (YP >= Height) then Exit;
   if not Compare(Pixels[YP,XP], Old) then Exit;

   L[0].X := XP; X1 := XP;
   L[0].Y := YP; Y1 := YP;
   LS := 1;
   RS := 0;
   while (LS > 0) or (RS > 0) do begin
      if LS > 0 then begin
         if LS > MAX_STACK then LS := MAX_STACK;
         Dec(LS);
         X1 := L[LS].X;
         Y1 := L[LS].Y;
      end else if RS > 0 then begin
         if RS > MAX_STACK then RS := MAX_STACK;
         Dec(RS);
         X1 := R[RS].X;
         Y1 := R[RS].Y;
      end;

      while (Y1 > 0) and Compare(Pixels[Y1-1,X1], Old) do Dec(Y1);

      LF := false;
      RF := false;
      while Y1 < Height do begin
         if Compare(Pixels[Y1,X1], Old) then begin
            Pixels[Y1,X1] := New;

            if X1 > 0 then begin
               if Compare(Pixels[Y1,X1-1], Old) then begin
...
procedure TFastDIB.DrawFrame(X1,Y1,X2,Y2 : integer; Color : TFColor);
begin
...
procedure TFastDIB.DrawFrameB(X1,Y1,X2,Y2 : integer; Color : byte);
begin
   LineBresenhamB(X1,Y1,X2,Y1,Color);
   LineBresenhamB(X1,Y2,X2,Y2,Color);
   LineBresenhamB(X1,Y1,X1,Y2,Color);
   LineBresenhamB(X2,Y1,X2,Y2,Color);
end;
...
procedure TFastDIB.LineBresenhamMirrorY(x0,y0,x1,y1 : integer; Color : TFColor);
begin
  LineBresenham(x0, Height-1-y0, x1, Height-1-y1, Color);
end;

procedure TFastDIB.LineBresenhamBMirrorY(x0,y0,x1,y1 : integer; Color : byte);
begin
  LineBresenhamB(x0, Height-1-y0, x1, Height-1-y1, Color);
end;

function  TFastDIB.DrawEllipseMirrorY(CX, CY, XRadius, YRadius : integer; Color : tfColor; Fill : boolean = false) : integer;
begin
  Result := DrawEllipse(CX, Height-1-CY, XRadius, YRadius, Color, Fill);
end;

function  TFastDIB.DrawEllipseBMirrorY(CX, CY, XRadius, YRadius : integer; Color : byte) : integer;
begin
  Result := DrawEllipseB(CX, Height-1-CY, XRadius, YRadius, Color);
end;

procedure TFastDIB.DrawFrameMirrorY(X1,Y1,X2,Y2 : integer; Color : TFColor);
begin
  DrawFrame(X1, Height-1-Y1, X2, Height-1-Y2, Color);
end;

procedure TFastDIB.DrawFrameBMirrorY(X1,Y1,X2,Y2 : integer; Color : byte);
begin
  DrawFrameB(X1, Height-1-Y1, X2, Height-1-Y2, Color);
end;

procedure TFastDIB.DrawArcMirrorY(CX, CY, XRadius, YRadius, X1,Y1, X2,Y2 : integer; Color : tfColor);
begin
  DrawArc(CX, Height-1-CY, XRadius, YRadius, X1, Height-1-Y1, X2, Height-1-Y2, Color);
end;

procedure TFastDIB.FloodFillMirrorY(XP,YP : integer; Old, New : tfColor);
begin
  FloodFill(XP, Height-1-YP, Old, New);
end;


//----------------------------------------------------------------------------

procedure TFastDIB.LoadFromResource(hInst:HINST; Name : string);
var s   : integer;
    h,r : THandle;
    p   : PChar;
    Buffer : Pointer;
    ...
    fBits,xSize,fSize: DWord;
begin
  h := FindResource(HInstance,PChar(Name),RT_RCDATA);
  r := LoadResource(HInstance, h);
  p := LockResource(r);
  s := SizeofResource(HInstance,h);
  ...
  fBits:=LoadHeader(Buffer,bmInfo);
  SetSizeIndirect(bmInfo);
  Buffer := Pointer(Integer(p)+fBits);
  if(bmInfo.Header.Compression=1)or(bmInfo.Header.Compression=2)then xSize:=PDWord(Integer(Buffer)+2)^-fBits else
  if fSize-fBits > Size then xSize:=Size else xSize:=fSize-fBits;
  if(bmInfo.Header.Compression=0)or(bmInfo.Header.Compression=3)then begin
    Move(Buffer^,Bits^,xSize);
  end else begin
    if bmInfo.Header.Compression=1 then DecodeRLE8(Self,Buffer) else DecodeRLE4(Self,Buffer);
  end;
