...
procedure THistogramForm.sbtnCancelClick(Sender: TObject);
begin
  Modalresult := mrCancel;
end;

procedure THistogramForm.sbtnOkClick(Sender: TObject);
begin
  Modalresult := mrOk;
end;
...
begin
  For i:=0 to 2 do begin
   ...
   For j:=0 to 255 do
    if Colors[i,j]>0 then
       Chart1.Series[I].AddXY(J,Colors[i,j],'',Chart1.Series[i].SeriesColor)
    else
  ...
  Co3 := 0;
   ...
   For j:=0 to 255 do begin
    ...
    For i:=0 to 2 do begin
        CoXY := Colors[i,j];
        Co3  := Co + (GetRValue(CoXY)+GetGValue(CoXY)+GetBValue(CoXY));
//        Co := Co + Colors[i,j];
    end;
    if Co3>0 then
       Chart1.Series[0].AddXY(J,Co3,'',clBlack)
