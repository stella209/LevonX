...
(* USB -> Srial comminication interface rutins
   By Agócs László Hungary - 2009)
*)
interface
...
Var
  ...
  passed : boolean;          // Indicates the USB drive present
  USB_Names : Names;         // USB Drivers list
  USBName   : string;        // USB Driver name
  USBData: byte;             // Data byte for read/write;
  oldUSBData: byte;          // Data byte for read/write;
  Data   : byte;             // USB data byte
...
if (name<>'') and (name = DualName) then
  begin
...
if passed then
  begin
  res := Set_USB_Device_LatencyTimer(2);
...
if passed then
  ...
  res := Set_USB_Device_BitMode(data,1); // Asyncron mode
  write_value := (res = FT_OK);
  end;
end;
