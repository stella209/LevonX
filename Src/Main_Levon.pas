...
begin
  ...
  if SKALAZASForm.Execute(ALZ.WorkBMP,ALZ.sCent,ALZ.Zoom) then begin
     ...
     ALZ.ReDraw;
     UR.UndoSave;
     StatusBar1.Panels[3].Text := FpicFile;
...
begin
  ...
  BMPCopy( ALZ.WorkBMP, cBook[ cBookIndex ] );
end;
...
begin
  ...
  if UjForm.ModalResult = mrOk then begin
     if UjForm.PictureStay then begin
        BMPResize(ALZ.WorkBMP,UjForm.PictureWidth,UjForm.PictureHeight);
//        ALZ.OrigBMP.Assign(ALZ.WorkBMP);
     end else begin
        ALZ.New(UjForm.PictureWidth,UjForm.PictureHeight,UjForm.PictureColor);
     end;
     UR.UndoSave;
  end;
end;
