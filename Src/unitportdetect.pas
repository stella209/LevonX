...
// StellaSOFT add
function IsUSBSerialPort(n: byte): boolean;
function GetUSBSerialPort: byte;
function GetBlueToohPorts(var ports: array of TDetectedPort): byte;
...
function IsUSBSerialPort(n: byte): boolean;
var
  comPorts: array[0..254] of TDetectedPort;
  pName: string;
begin
  Result := False;
  if n>0 then
  begin
       ...
       pName := comPorts[n-1].friendlyName;
       if pName<>'' then
          Result := (Pos('USB',pName)>0) and
               ((Pos('Serial',pName)>0) or
               (Pos('Soros',pName)>0));
  end;
end;
...
begin
  ...
  for i := 0 to 254 do begin
      pName := comPorts[i].friendlyName;
      if pName<>'' then
      with comPorts[i] do
         if (Pos('USB',pName)>0) and
            ((Pos('Serial',friendlyName)>0) or
             (Pos('Soros',friendlyName)>0))
         then begin
            Result := i+1;
            exit;
         end;
...
begin
  ...
  getCOMPorts(Ports);
  for i := 0 to 254 do begin
      pName := ports[i].friendlyName;
      if pName<>'' then
         if Pos('Bluetooth',pName)>0 then begin
            Result := i+1;
         end else
         begin
...
var
  ...
  fnbuffer: array[0..255] of AnsiChar;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          instanceId := buffer;
          if SetupDiGetDeviceRegistryPropertyA(devInfoList, @devInfoData, SPDRP_FRIENDLYNAME, nil, @fnbuffer, sizeof(fnbuffer), nil) then
            friendlyName := fnbuffer
          else if SetupDiGetDeviceRegistryPropertyA(devInfoList, @devInfoData, SPDRP_DEVICEDESC, nil, @fnbuffer, sizeof(fnbuffer), nil) then
            friendlyName := fnbuffer
          else
            friendlyName := '';
