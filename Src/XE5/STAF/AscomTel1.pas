...
uses
  ...
  Forms, Dialogs, ShellAPI, Variants,
...
Procedure ScopeAlign(ra,dec : double); stdcall;
...
var
  ...
  T,Ser : Variant;                        // Telescope variant
...
with pop_scope do begin
   if ScopeInitialized then begin
      ...
      pos_x.text := ARToStr_(Curdeg_x/15);
      pos_y.text := DEToStr_(Curdeg_y);
      if ShowAltAz.checked then begin
          az_x.text  := DEToStr_(Cur_az);
          alt_y.text := DEToStr_(Cur_alt);
         end else begin
          az_x.text  := '';
          alt_y.text := '';
      end;
...
latitude:=la;
longitude:=-lo;
pop_scope.lat.text:=DEToStr_(latitude);
pop_scope.long.text:=DEToStr_(longitude);
end;
